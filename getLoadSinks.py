...
if __name__ == "__main__":
    ...
    for i in range(len(inputFiles)):
        ...
        for x in bs2data:
            ...
            '''
            if x[0]=='311:480:15fdf':
                for t in sorted(x[1].keys()):
                    print t
                    print "------------"
                    for u in x[1][t]:
                        print str(u) + " " +  str(sorted(x[1][t][u]))
            '''

            getLoad(x[0],x[1],bs2time2activeUEs,intervals,bs2prevTimeStats)
            '''
            if x[0]=='311:480:15fdf':
                for t in sorted(bs2time2activeUEs[x[0]].keys()):
                    print t
                    print "------------"
                    print bs2time2activeUEs[x[0]][t]
            '''

    i = 0
    for bs in bs2time2activeUEs:
        i += 1
        getNeighborGradients(bs,bs2neighbors[bs],bs2time2activeUEs,bs2time2neighbor2flow)
        #print i
