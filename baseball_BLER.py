    ...
    for imsi in imsi2time2recs:
         for i in range(intervalBoundary[0],intervalBoundary[1]+1):
              ...
              if t in imsi2time2recs[imsi]:
                   ...
                   v[i] = (v[i][0]+numCtxReleases+1,v[i][1]+cumBLER,v[i][2]+numBLERSamples)
    eNodeBLoadVec[idx] += v
...
def generateBS2Data(line):
    ...
    return (curBS,{imsi: {T: [(startTime,procID,cfc,sum_BLER,numBLERSamples)]}})
...
if __name__ == "__main__":
    ...
    for i in range(1,len(intervals)):
        ...
        for v in V:
            bearerLoad = v[i][0]
            if v[i][2]>0:
                failureRate = float(v[i][1])/v[i][2]
            else:
                failureRate = 0.0
            #failureV.append(failureRate)
            #f3 = getRollingAve(failureV,3)
            #f5 = getRollingAve(failureV,5)
